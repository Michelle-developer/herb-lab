...
function HerbCardMenu({ cardGroup, item }) {
  ...
  return (
    <div className={clsx('absolute inset-0 bg-black/60 md:flex', `${cardGroup}:visible`)}>
      {/* 上面用的： invisible */}
      <div className="mt-2 flex w-full items-start justify-around gap-2">
        {/**onClick={handleMoveBetweenFiles} */}
        <button className="flex gap-0.5 rounded-full bg-stone-700 p-2 text-sm text-stone-200 hover:cursor-pointer hover:bg-stone-900 hover:text-stone-50">
          <MoveLeft size={16} /> <span className="">移動</span>
        </button>
        <button
          onClick={() => handleRemove(item.herbId._id, tempFolder._id)}
          className="flex gap-0.5 rounded-full bg-stone-700 p-2 text-sm text-rose-600 hover:cursor-pointer hover:bg-stone-900 hover:text-rose-400"
        >
          <CircleX size={16} /> 刪除
        </button>
      </div>
